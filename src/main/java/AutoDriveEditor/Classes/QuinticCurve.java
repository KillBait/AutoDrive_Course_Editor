...
public class QuinticCurve {
    ...
    private final Point2D.Double virtualControlPoint4;
    ...
    public QuinticCurve(MapNode startNode, MapNode endNode) {
        ...
        if (this.numInterpolationPoints < 2) {
            this.numInterpolationPoints = 2;
        }

        // Create four control points. Adjust initial positions as needed.
        ...
        this.controlPoint4 = createMapNode(3, (startNode.x + endNode.x) / 2, 0, startNode.z, NODE_FLAG_CONTROL_POINT, false, true);
        ...
        this.virtualControlPoint4 = new Point2D.Double(controlPoint4.x, controlPoint4.z);
    ...
    public void getInterpolationPointsForCurve(MapNode startNode, MapNode endNode) {
        ...
        double step = 1.0 / this.numInterpolationPoints;
        ...
        int id = 0;        
        // Compute in-between points using the quintic Bézier formula:
		// Note: using (i+step < 1.0001) to account for rounding errors.
        // B(t) = (1-t)^5 * P0 + 5(1-t)^4 * t * CP1 + 10(1-t)^3 * t^2 * CP2 +
        //        10(1-t)^2 * t^3 * CP3 + 5(1-t)* t^4 * CP4 + t^5 * P5
        for (double i = step; i + step < 1.0001; i += step) {
            Point2D.Double point = calcPointsForCurve(startNode, endNode,
                    virtualControlPoint1.x, virtualControlPoint1.y,
                    virtualControlPoint2.x, virtualControlPoint2.y,
                    virtualControlPoint3.x, virtualControlPoint3.y,
                    virtualControlPoint4.x, virtualControlPoint4.y,
                    i);
	 ...
	 * @param cp4x x-coordinate for control point 4
     * @param cp4z z-coordinate for control point 4
    ...
    public Point2D.Double calcPointsForCurve(MapNode startNode, MapNode endNode,
        ...
        point.x = oneMinusT5 * startNode.x +
                  5 * oneMinusT4 * t * cp1x +
                  10 * oneMinusT3 * t2 * cp2x +
                  10 * oneMinusT2 * t3 * cp3x +
                  5 * oneMinusT * t4 * cp4x +
                  t5 * endNode.x;
        point.y = oneMinusT5 * startNode.z +
                  5 * oneMinusT4 * t * cp1z +
                  10 * oneMinusT3 * t2 * cp2z +
                  10 * oneMinusT2 * t3 * cp3z +
                  5 * oneMinusT * t4 * cp4z +
                  t5 * endNode.z;
        return point;
    }
    ...
    public void commitCurve() {
        ...
        if (bDebugLogCurveInfo) LOG.info("QuinticCurve created {} nodes", mergeNodesList.size() - 2);
    }
    ...
    public void clear() {
        ...
        this.controlPoint4 = null;
        this.curveStartNode = null;
        this.curveEndNode = null;
    }

    // Update virtual control points (for smooth dragging/scaling)
    public void updateVirtualControlPoint1(double diffX, double diffY) {
        if (buttonManager.getCurrentButtonID().equals("Quintic")) {
    ...
    public void updateVirtualControlPoint4(double diffX, double diffY) {
        if (buttonManager.getCurrentButtonID().equals("Quintic")) {
            this.virtualControlPoint4.x += diffX * controlPointMoveScaler;
            this.virtualControlPoint4.y += diffY * controlPointMoveScaler;
        } else {
            this.virtualControlPoint4.x += diffX;
            this.virtualControlPoint4.y += diffY;
        }
        this.updateCurve();
    }

    // Update control point positions and trigger curve update
    ...
    public void updateControlPoint4(double diffX, double diffY) {
        this.controlPoint4.x = roundUpDoubleToDecimalPlaces(this.controlPoint4.x + diffX, 3);
        this.controlPoint4.z = roundUpDoubleToDecimalPlaces(this.controlPoint4.z + diffY, 3);
        updateVirtualControlPoint4(diffX, diffY);
        updateCurve();
    }

    // Methods to move control points with grid snapping
    ...
    public void moveControlPoint4(double diffX, double diffY) {
        Point2D point = calcScaledDifference(this.controlPoint4, diffX, diffY);
        this.controlPoint4.x = roundUpDoubleToDecimalPlaces(this.controlPoint4.x + point.getX(), 3);
        this.controlPoint4.z = roundUpDoubleToDecimalPlaces(this.controlPoint4.z + point.getY(), 3);
        updateVirtualControlPoint4(point.getX(), point.getY());
        updateCurve();
    }
    ...
    public boolean isControlPoint(MapNode node) { 
        return node == this.controlPoint1 || node == this.controlPoint2 || node == this.controlPoint3 || node == this.controlPoint4; 
    }
    ...
    public MapNode getControlPoint4() { 
